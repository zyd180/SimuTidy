...
- **分支线（3.4.1 起）**：一条线分出多个目标时，Goto 只建 1 个（源端），每个可行分支各建 1 个 From（同一 Tag）；空间不足的分支**保留原连线不动**（部分拆分会在结果面板说明），全部分支都放不下才整线跳过
...
2. **拆分 Goto/From 的分支线**（3.4.1 起支持）：一条线分出多个目标时逐分支处理，空间不足的分支保留原连线并在结果面板说明；仅全部分支均无空间时整线跳过

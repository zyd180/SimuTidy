...
| **信号线命名** | 按源模块名、端口号或输出端口自动命名信号线；支持清除所选/全部命名（全部清除带二次确认） |
| **Goto/From 拆分** | 选中信号线批量拆分为 Goto/From 对（支持多选与分支线，空间不足自动推块） |
| **信号对象解析** | 当前层级有名字的信号线批量勾选"信号名称必须解析为 Simulink 对象" |
| **高亮未连接端口** | 高亮显示模型中未连接的端口（含分类原因与逐条定位） |
| **模块重叠检测** | 检查当前层级两两重叠的模块，结果面板逐条定位 |
| **Goto/From 配对诊断** | 检查悬空 Goto、无源 From、跨层 local 标签引用 |
